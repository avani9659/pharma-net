...
# fetchChannelConfig <org> <channel_id> <output_json> <peer num>
...
fetchChannelConfig() {
  ...
  set -x
  peer channel fetch config config_block.pb -o orderer.pharma-network.com:7050 --ordererTLSHostnameOverride orderer.pharma-network.com -c $CHANNEL --tls --cafile "$ORDERER_CA"
  { set +x; } 2>/dev/null
...
# signConfigtxAsPeerOrg <org> <configtx.pb> <peer num>
