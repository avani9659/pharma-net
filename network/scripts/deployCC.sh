...
## Install chaincode on all the peers of all the organizations
installChaincodeToAllPeers() {
	FABRIC_CFG_PATH=$PWD/../config/	
...
queryInstalledToAllPeers() {
	for org in "${ORGS[@]}"; do
		for peer in 0 1; do 
      infoln "checking if chaincode Installed  on *****peer$peer.$org*****..."
...
## Approve for all the orgs. This can be done using only 1 peer of each organization
approveForMyOrg "manufacturer" 0
approveForMyOrg "distributor" 0
approveForMyOrg "retailer" 0
approveForMyOrg "consumer" 0
approveForMyOrg "transporter" 0

## check whether the chaincode definition is ready to be committed
commitReadinessCheck(){
  echo "=========================== starting commit readiness ============================"
...
chaincodeQueryCommitted(){
  echo "=========================== executing query committed ========================"
