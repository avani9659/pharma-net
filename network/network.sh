...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates for manufacturer..."
    fi
    ...
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates for distributor..."
    fi

    #update for other orgs
    ...
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates for consumer..."
    fi
    ...
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates for transporter..."
    fi
  ...
  infoln "Generating CCP files for manufacturer, Distributor, retailer, consumer and transporter"
...
# call the script to create the channel, join the peers of manufacturer and distributor,
...
function createChannel() {
  ...
  echo "running cmd -scripts/createChannel.sh $CHANNEL_NAME $CLI_DELAY $MAX_RETRY $VERBOSE"
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    #update for all the peers
    # ${CONTAINER_CLI} volume rm docker_orderer.pharma-network.com docker_peer0.manufacturer.pharma-network.com docker_peer1.manufacturer.pharma-network.com docker_peer0.distributor.pharma-network.com docker_peer1.distributor.pharma-network.com docker_peer0.retailer.pharma-network.com docker_peer1.retailer.pharma-network.com docker_peer0.consumer.pharma-network.com docker_peer1.consumer.pharma-network.com docker_peer0.transporter.pharma-network.com docker_peer1.transporter.pharma-network.com
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/transporter/msp organizations/fabric-ca/transporter/tls-cert.pem organizations/fabric-ca/transporter/ca-cert.pem organizations/fabric-ca/transporter/IssuerPublicKey organizations/fabric-ca/transporter/IssuerRevocationPublicKey organizations/fabric-ca/transporter/fabric-ca-server.db'
...
COMPOSE_FILE_BASE=compose-pharma-network.yaml
